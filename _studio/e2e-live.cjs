...
const B = 'http://localhost:8204'; const W = '0x00000000000000000000000000000000000000e1';
...
(async () => {
  await sleep(2000);
  const cfg = await get('/api/config'); ok('config: live open, treasury published, min deposit 10', cfg.live === true && /^0x[0-9a-f]{40}$/.test(cfg.treasury) && cfg.minDeposit === 10);
  const lv = await get('/api/live'); ok('/api/live exposes treasury + chain read', lv.open && lv.treasury === cfg.treasury && 'treasuryUsdc' in lv.chain);
